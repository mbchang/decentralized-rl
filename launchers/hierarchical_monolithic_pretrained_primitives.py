...
def parse_args():
    parser = build_parser(transformation='subpolicy', pretrain_primitives=True)
    args = parser.parse_args()
    args.hrl = True
    return args


class HierarchicalPretrainedMonolithicLauncher(HierarchicalMonolithicLauncher):
    ...
    def load_primitive_weights(cls, organism, ckpts, pfunc):
        ...
        for i, (t_id, ckpt) in enumerate(zip(organism.transformations, ckpts)):
            transformation = organism.transformations[t_id]
            transformation.load_state_dict(ckpt['organism'])
            transformation.set_trainable(not organism.args.freeze_primitives)
...
if __name__ == '__main__':
    launcher = HierarchicalPretrainedMonolithicLauncher()
